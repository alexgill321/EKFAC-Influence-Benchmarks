...
class BaseObjective(abc.ABC):
    """An abstract adapter that provides torchinfluence with project-specific information
    about how training and test objectives are computed.

    In order to use torchinfluence in your project, a subclass of this module should be
    ...
    def train_loss(self, model: nn.Module, params: torch.Tensor, batch: Any) -> torch.Tensor:
        ...
        This method should not be overridden for most use cases. By default, torchinfluence
...
class BaseInfluenceModule(abc.ABC):
    ...
    def stest(self, test_idxs: List[int]) -> torch.Tensor:
        ...
        #
        # print("loss grad on the test example is {}".format(self.test_loss_grad(test_idxs).shape))
        # exit()
    ...
    def influences(
        ...
        # stest is IHVP with the test grad example.
    ...
    def _model_make_functional_single_param(self):

        assert not self.is_model_functional
        param_tuple = self._model_params(with_names=True)

        only_required_weight_param = tuple(param_tuple[2][1])
        changed_param_name = param_tuple[2][0]

        # params = tuple(p.detach().requires_grad_() for p in self._model_params(with_names=True))
        # print(param_tuple[2][0])

        for name in self.params_names:
            if name == changed_param_name:
                _del_attr(self.model, name.split("."))
        self.is_model_functional = True

        return only_required_weight_param
    ...
    def _loss_grad_loader_wrapper(self, train, **kwargs):
        ...
        flat_fc2_param = params[2].view(-1)
        ...
        for batch, batch_size in self._loader_wrapper(train=train, **kwargs):
            ...
            yield self._flatten_params_like(torch.autograd.grad(loss, params[2])), batch_size
