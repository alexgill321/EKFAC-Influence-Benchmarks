    ...
    def _compute_EKFAC_GNH(self, n_samples: int = 2):
        ...
        for group in ekfac.param_groups:
            with autocast():
                ...
                print('Expanding eigenvectors')
                # Compute Kronecker product of eigenvalues and eigenvectors

                print("Expanding eigenvalues")
                print(f"activation eigenvalues shape: {la.shape}")
                print(f"layer output eigenvalues shape: {ls.shape}")
                print('calculating eigenvalue kron')
                eigenval_diags = torch.outer(la, ls).flatten(start_dim=0)
                print(f'Eigenvalue diagonal product shape {eigenval_diags.shape}')

                print(f'Qa type: {Qa.type()}')

                eigenvec_kron = torch.kron(Qa, Qs)

                print(f'Eigenvalue kron shape {eigenvec_kron.shape}')

                print("G approximation Matrix mult")
                # Compute GNH
                G_approx = torch.matmul(eigenvec_kron, torch.matmul(eigenval_diags, eigenvec_kron.t()))
                print(f'G approximation shape: {G_approx.shape}')
                G_list.append(G_approx)
