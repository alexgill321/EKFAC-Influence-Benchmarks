...
if __name__ == '__main__':
    ...
    pbrf_files = [filename for filename in os.listdir(os.getcwd() + '/results/') if
                  filename.startswith('pbrf_influences_fc2_')]

    print("getting all EKFAC scores")

    ekfac_files = [filename for filename in os.listdir(os.getcwd() + '/results/') if
                   filename.startswith('ekfac_influences_fc2_')]
    ...
    for ekfac_file, pbrf_file in pairs:
        try:
            ...
            corr_with_retrain_pbrf = influence_correlation(os.getcwd() + '/results/' + pbrf_file, os.getcwd() + '/pbrf/retrain_pbrf/retrained_pbrf_scores_fc1.txt')
            corr_with_retrain_ekfac = influence_correlation(os.getcwd() + '/results/' + ekfac_file, os.getcwd() + '/pbrf/retrain_pbrf/retrained_pbrf_scores_fc1.txt')
            all_corr_data.append([ekfac_damp, pbrf_damp, gnh, corr, corr_with_retrain_pbrf, corr_with_retrain_ekfac])
        except Exception as e:
            print(e)

        column_names = ['ekfac_damp', 'pbrf_damp', 'pbrf_gnh', 'correlation', 'correlation_with_retrain_pbrf', 'correlation_with_retrain_ekfac']
